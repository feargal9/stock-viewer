...
describe("Stock View", () => {
  ...
  // it("can clear a dropdown value", async () => {
  //   const { queryByTestId, queryByText, findByTestId, debug } = renderComponent(SYMBOLS_REQUEST, SYMBOLS_RESPONSE, { name: "symbols-path" });
  //   await waitForElementToBeRemoved(() => queryByTestId("loader"));
  //   debug();
  //   const select = queryByText("/Select a stock/") as HTMLInputElement;
  ...
  //   debug();
  //   expect(select.value).toBe("");
  // const select = await findByTestId("select") as HTMLInputElement;
  // console.log("firing");
  // fireEvent.change(select, { target: { value: "DERP" } });
  // await wait(() => {
